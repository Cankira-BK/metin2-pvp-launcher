...
class VirusTotalAPIV2 {
	...
	public function GetVerboseMessage($report) {
		if (!isset($report->response_code)) return -1;
		if ($report->response_code != 1) return -1;
		if (!isset($report->verbose_msg)) return -1;
		
		if ($report->verbose_msg == "Scan request successfully queued, come back later for the report")
			return "Tarama isteği başarıyla sıraya alındı!";
		else
			return $report->verbose_msg;
	}

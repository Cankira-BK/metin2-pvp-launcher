$(document).ready(function(){
  ...
  var OnlinePlayerCount = parseInt(document.getElementById("onlineCount").innerHTML);
  function OnlinePlayerCounter()
  {
    data = {"total":number,"max":20000};
    data.total += Math.floor((Math.random() * 100) + 1);
    percentCalc = (data.total+Math.floor((Math.random()) * 1500 + 1)) * 100 / data.max;
...
$(document).ready(function () {
  ...
  $('a.itemshop').fancybox({
    ...
    'onStart': function () {
      ...
      $('#fancybox-outer').css({'background': 'transparent url("'+$('body').data("url")+'assets/images/'+fancy_c+'.png") center center no-repeat'});
      $('#fancybox-close').css({

<?php if (Functions::checkAllPermission($_SESSION['allPermissionArray'],"81") == true):?>
<?php
		...
		<div class="portlet light portlet-fit bordered">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green bold uppercase">Bakım Modu Ayarları</span>
				</div>
			</div>
			<div class="portlet-body">
				<form action="<?=URI::get_path('site/care');?>" method="post">
				<div class="form-body">
					<div class="form-group form-md-line-input form-md-floating-label">
						<select class="form-control edited" name="status">
							<?php foreach ($captchaStatus as $key=>$row):?>
								<?php if ($key == \StaticDatabase\StaticDatabase::settings('maintenance')):?>
						...
						<label for="form_control_1">Bakım Durumu </label>
					</div>
				</div>
								...
								<div class="mt-card-item">
									...
									<div class="mt-card-content">
										...
										<div class="mt-card-social">
											<ul>
												<?php if(\StaticDatabase\StaticDatabase::settings('caretheme') == $file){?>
												<li><button id="install_<?php echo $file;?>" type="button" disabled class="btn btn-success">Varsayılan</button></li>
												<?php }else{ ?>
												<li><button type="submit" data-style="contract" name="care" value="<?=$file;?>" class="btn btn-primary">Varsayılan olarak ayarla</button></li><?php } ?>
											</ul>
										</div>
									</div>
								</div>

							...
							<?php if (Cache::check('guilds')):?>
								<?php foreach ($this->views['data'] as $key=>$row):?>
                                    <tr class="c1">
                                        ...
                                        <td class="pname"><a href="<?=URI::get_path('detail/guild/'.$row['lonca'])?>"><?=$row['lonca']?></a></td>
                                        <td class="pname"><?=$row['ladder_point'];?></td>
                                        <td class="pname"><img src="<?=URL.'data/flags/'.$row['bayrak'].'.png'?>" alt="<?=\StaticDatabase\StaticDatabase::settings('oyun_adi')?>"></td>
                                        <td class="pname"><a href="<?=URI::get_path('detail/player/'.$row['baskan'])?>"><?=$row['baskan'];?></a></td>
                                        <td class="pname">
                                            <span class="label label-green"><?=$row['win']?></span> | <span class="label label-draw"><?=$row['draw']?></span> | <span class="label label-red"><?=$row['loss']?></span>
                                        </td>
                                    </tr>
								<?php endforeach;?>
							<?php endif;?>

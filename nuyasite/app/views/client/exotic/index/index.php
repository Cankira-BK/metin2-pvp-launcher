...
<div style="float: left; width: 665px; margin-left:20px;">
    <div style="float: left; margin-top: 10px;">
        ...
        <div class="windows windows-wbCenter">
            <div class="content">
                <span class="title"><?=$lng[62]?></span>
				<?php if (\StaticDatabase\StaticDatabase::settings('facebook_status')):?>
                    <!--Facebook Haber Start-->
                    <div id="fb" class="bg-light" style="margin-bottom: 15px;margin-top: 15px">
				...
				<?php if (Cache::check('patch')): ?>
					<?php foreach ($this->result['patch'] as $row): ?>
                        <div class='newspaper'>
                            <a href='<?= URI::get_path('patch/view/' . $row['id']) ?>"><?= $row['title']; ?>'><span
                                        class='newstitle'><?= $row['title']; ?></span></a>
                            <span class='newssender'><?=$lng[64]?> : <?= Functions::zaman($row['tarih']) ?></span>
                            <span class='newssmallcontent'><?= $row['content2']; ?></span>
                            <a href='<?= URI::get_path('patch/view/' . $row['id']) ?>'
                               class='wbuttons wbuttons-btDevam '></a>
                        </div>
					<?php endforeach; ?>
				<?php endif; ?>
        ...
        <center>
            ...
            <a href="<?=\StaticDatabase\StaticDatabase::settings('facebook')?>" target="_blank">
                <div class="facebook-like">
                    <img src="<?=URI::public_path('assets/images/face.png')?>" alt="">

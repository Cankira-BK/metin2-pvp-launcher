            ...
            <div class="main-text-bg">
                <div class="main-text">
                    ...
                    <?php if ($ban['status'] == 'OK' && $availDt <= $now):?>
                        ...
                        <?php else:?>
                            <?php if ($ban['permitted_mac_address'] == 0):?>
                                <?=Client::alert('error',"Güvenli PC özelliğiniz pasif durumda.Lütfen önce Güvenli PC özelliğini aktifleştiriniz."); ?>
                            <?php else:?>
                                <?php
                                ...
                                if($cError == 'recaptcha'){
                                    ...
                                    echo Client::alert('success','Yeni Bilgisayar Ekleme linki mail adresinize gönderildi. Lütfen kontrol ediniz.');
                                    Session::set('cError',false);
                                }elseif($cError == 'time'){
                                    $now = date('i');
                                    ...
                                    echo Client::alert('info','Yeni Bilgisayar Ekle tuşuna bastığınızda sistemde kayıtlı mail adresinize link gönderilecektir. Lütfen linki takip ederek depo şifrenizi değiştiriniz.');
                                }
                                ?>
                                <form action="<?=URI::get_path('profile/securenew2')?>" method="POST">
                                    <div class="bg-light">
                                        <table>
                                            <tbody>
                                            <tr>
                                        ...
                                        <button type="submit" class="btn">Yeni Bilgisayar Ekle</button>
                                    </div>
                                </form>
                            <?php endif;?>
                        <?php endif;?>

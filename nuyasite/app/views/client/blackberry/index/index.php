<div id="content_ajax" class="haberler">
    <h1 class="YoutubeTitle">Haberler</h1>
	<?php Cache::open('patch');?>
	<?php if (Cache::check('patch')):?>
		<?php foreach ($this->result['patch'] as $news):?>
            <div class="panel panel-buyuk Renk-1">
                <div class="panel-heading"><a href="<?=URI::get_path('patch/view/'.$news->id)?>"><?=$news->title;?></a></div>
                <div class="panel-body haber-body">
                    <span class="haber-tarih"><i class="fas fa-calendar-alt" aria-hidden="true"></i>&nbsp;&nbsp;   <?=Functions::prettyDateTime1($news->tarih)?> </span>
                    <p style="padding: 15px"><?=$news->content2;?></p>
                    <a class="green-button news-btn" href="<?=URI::get_path('patch/view/'.$news->id)?>"><?=$lng[63]?></a>
                </div>
            </div>

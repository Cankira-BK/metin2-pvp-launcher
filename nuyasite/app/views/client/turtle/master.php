					...
					<div class="container">
						<aside class="left-sidebar">
							<div class="download-block">
								<a href="<?=URI::get_path('download')?>"><span><?=$lng[0]?></span></a>
							</div>
							<?php if (isset($_SESSION['aid'])):?>
							<div class="widget login-block red-light p-block">
                                <div class="widget-title flex-s-c">Kullanıcı Paneli</div>
                                <span class="master-profile">
                                    <?=$lng[22]?> <b><?=Session::get('cLogin')?></b><br>
									<?=$lng[17]?> <b><?=$getAin[CASH]?> EP</b><br>
									<?=$lng[17]?> <b><?=$getAin[MILEAGE]?> EM</b>
                                </span>

								<div class="kayip-buttonlar2" style=" margin-top: 15px;">
									<a href="<?=URI::get_path('profile')?>"><?=$lng[19]?></a>
									<a href="<?=URL.SHOP?>" class="btn btn-giris itemshop itemshop-btn iframe"><?=$lng[147]?></a>
									<a href="<?=URL.MUTUAL?>" class="btn btn-giris itemshop itemshop-btn iframe"><?=$lng[146]?></a>
									<a href="<?=URI::get_path('login/logout')?>"><?=$lng[20]?></a>
								</div>
							</div>
							<?php else:?>
							<div class="widget login-block red-light p-block">
                                <div class="widget-title flex-s-c"><?=$lng[21]?></div>
								<form action="<?=URI::get_path('login/control')?>" id="LoginNotifyOGs" method="post" accept-charset="utf-8" onkeypress="return event.keyCode != 13;">
									...
									<div class="login-button">
										...
										<a href="<?=URI::get_path('recuperare')?>" data-hasevent="1" style="float: right;position: relative;top: 2px;"><?=$lng[25]?></a>
										<a href="<?=URI::get_path('recuperare/account')?>" data-hasevent="1" style="float: right;position: relative;top: -12px;"><?=$lng[26]?></a>
									</div>
								</form>
							</div>
							<?php endif;?>

							<div class="top-players-block light p-block widget red-dark">

...
<div class="panel panel-buyuk Renk-2">
    <div class="panel-heading">Oyunu İndir
    </div>
        ...
        <table class="sidebar_rank">
            ...
            <?php foreach ($this->pack as $key => $row):?>
					<tr>
                        ...
                        <td><?=$row['name'];?></td>
                        <td><?=$row['size'];?></td>
                        <td><a href="<?=$row['url']?>" target="_blank"><img src="<?=$row['image']?>" alt="<?=\StaticDatabase\StaticDatabase::settings('oyun_adi')?>" width="70px;"></a></td>
                        <td><a href="<?=$row['url']?>" target="_blank" class="green-button" style="padding: 3px;margin-right: 15px;"><?=$lng[175]?></a></td>
                    </tr>
			<?php endforeach;?>
			</tbody>
        </table>

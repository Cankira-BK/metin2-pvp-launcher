...
<div class="panel panel-buyuk">
    ...
    <div class="panel-body">
		...
		<div class="kayip-buttonlar2-cp">
			...
			<a href="<?=URI::get_path('profile/email')?>" class="btn btn-giris" style="margin-left: 15px;"><i class="glyphicon glyphicon-envelope"></i> &nbsp; <?=$lng[142]?></a>
			<a href="<?=URI::get_path('profile/depo')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; <?=$lng[143]?></a>
			<a href="<?=URI::get_path('profile/save')?>" class="btn btn-giris" style="margin-left: 15px;"><i class="glyphicon glyphicon-hourglass"></i> &nbsp; <?=$lng[145]?></a>
			<a href="<?=URI::get_path('profile/ksk')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; <?=$lng[144]?></a>
			<?php if(\StaticDatabase\StaticDatabase::systems('pin_durum') === "1"):?>
				<a href="<?=URI::get_path('profile/pin')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; Pin Değiştir</a>
			<?php endif;?>
			<?php if(\StaticDatabase\StaticDatabase::systems('itemkilit_durum') === "1"):?>
				<a href="<?=URI::get_path('profile/iks')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; İtem Kilit Değiştir</a>
			<?php endif;?>
			<?php if(\StaticDatabase\StaticDatabase::systems('hesapkilit_durum') === "1"):?>
				<a href="<?=URI::get_path('profile/hgs')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; Hesap Kilit Değiştir</a>
			<?php endif;?>
			<?php if(\StaticDatabase\StaticDatabase::systems('karakterkilit_durum') === "1"):?>
				<a href="<?=URI::get_path('profile/kgs')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; Karakter Kilit Değiştir</a>
			<?php endif;?>
			<?php if(\StaticDatabase\StaticDatabase::systems('guvenlipc_durum') === "1"):?>
				<a href="<?=URI::get_path('profile/gpc')?>" class="btn btn-giris"><i class="glyphicon glyphicon-folder-close"></i> &nbsp; GüvenliPC Pasifleştir</a>
			<?php endif;?>
			<a href="<?=URL.SHOP?>" class="btn btn-giris itemshop itemshop-btn iframe" style="margin-left: 15px;"><i class="glyphicon glyphicon-lock"></i> &nbsp; <?=$lng[147]?></a>
			<a href="<?=URL.MUTUAL?>" class="btn btn-giris itemshop itemshop-btn iframe"><i class="glyphicon glyphicon-comment"></i> &nbsp; <?=$lng[146]?></a>
			<a href="<?=URI::get_path('login/logout')?>" class="btn btn-giris" style="margin-left: 15px;"><i class="glyphicon glyphicon-log-out"></i> &nbsp; <?=$lng[148]?></a>

		</div>

		<?php foreach ($this->players['player'] as $key=>$row):?>
			<a href="<?=URI::get_path("detail/player/".$row['name'])?>">
				<div id="profileBox" class="player-table detail-section" style="float: left;width: 49%;">

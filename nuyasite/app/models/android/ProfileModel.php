<?php
    ...
    class ProfileModel extends IN_Model
    {
        ...
        public function index()
        {
			...
			$getGmList = $this->gdb->sql("SELECT mAccount FROM common.gmlist WHERE mAccount = ?",[$getPlayers['account']['login']]);
            if (count($getGmList) <= 0) { $getPlayers['modaration'] = false; } else {$getPlayers['modaration'] = true;}
            return $getPlayers;
        }

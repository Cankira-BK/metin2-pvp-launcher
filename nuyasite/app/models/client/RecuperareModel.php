    ...
    class RecuperareModel extends IN_Model
    {
        ...
        public function control()
        {
            ...
            if ($recaptcha == 1){
                ...
                if (!$captchaRes['success']) {
                    die(json_encode(["type"=>"danger","message"=>"Recaptcha doğrulaması gerçekleşmedi. Lütfen tekrar deneyiniz.","redirect"=>"".URL."recuperare/index"]));
                }
            }
			if (\StaticDatabase\StaticDatabase::settings('recaptcha') == 2)
			{
                ...
                if($login == '' || $email == ''){
                    die(json_encode(["type"=>"danger","message"=>"Hatalı kullanıcı adı veya email. Lütfen tekrar deneyiniz.","redirect"=>false]));
                    ...
                    if (count($control) <= 0){
                        ...
                        if ($now <= $tDate){
                            ...
                            $nows = date('i');
							$residuals = Session::get('residual');
							$kalan = $nows - $residuals;
							$kalans = 10 - $kalan;
							die(json_encode(["type"=>"info","message"=>"10 Dakika arayla mail gönderimi yapabilirsiniz.Lütfen bekleyiniz. Kalan süre : $kalans dakika","redirect"=>false]));
                        }else{
							...
							die(json_encode(["type"=>"success","message"=>"Şifre değişikliği linkiniz mail adresinize gönderildi.Lütfen mail adresinizi kontrol ediniz.","redirect"=>"".URL."recuperare/index"]));
                        }
                    }
                }
        }
        public function control2()
        {
            ...
            if ($recaptcha == 1){
                ...
                date_default_timezone_set('Europe/Istanbul');
                if (!$captchaRes['success']) {
                    die(json_encode(["type"=>"danger","message"=>"Recaptcha doğrulaması gerçekleşmedi. Lütfen tekrar deneyiniz.","redirect"=>"".URL."recuperare/account"]));
                }
            }
			if (\StaticDatabase\StaticDatabase::settings('recaptcha') == 2)
			{
                ...
                $email = $this->filter->mailFilter($_POST['email']);
                if($email == ''){
                    die(json_encode(["type"=>"danger","message"=>"Hatalı email. Lütfen tekrar deneyiniz.","redirect"=>false]));
                }elseif ($email == false){
                    die(json_encode(["type"=>"danger","message"=>"Hatalı email. Lütfen tekrar deneyiniz.","redirect"=>false]));
                }else{
                    ...
                    if (count($control) <= 0){
                        ...
                        if ($now <= $tDate){
							...
							die(json_encode(["type"=>"success","message"=>"Kullanıcı adı değişikliği linkiniz mail adresinize gönderildi.Lütfen mail adresinizi kontrol ediniz.","redirect"=>"".URL."recuperare/account"]));
                        }
                    }
                }
        }
                ...
                if($login == '' || $email == ''){
                    ...
                    if (count($control) <= 0){
                        ...
                        if ($now <= $tDate){
                            ...
                            $date = date('Y-m-d H:i:s');
                            $this->gdb->update('account',array('t_status'=>'1','t_key'=>$arg,'t_token'=>$token,'t_date'=>$date,'t_type'=>'4'),array('login'=>$login,'email'=>$email));
							die(json_encode(["type"=>"success","message"=>"Pin kodu değişikliği linkiniz mail adresinize gönderildi.Lütfen mail adresinizi kontrol ediniz.","redirect"=>"".URL."recuperare/pin"]));
                        }
                    }
                }
        }

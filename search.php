...
// Görüntülenme sayısı artır (AJAX request)
...
if (!empty($_GET['min_year'])) {
    $conditions[] = "CAST(year AS UNSIGNED) >= ?";
...
if (!empty($_GET['sort'])) {
    switch ($_GET['sort']) {
        ...
        case 'price_desc':
            ...
            break;
        case 'year_desc':
            $orderBy = "CAST(year AS UNSIGNED) DESC";
            break;
        case 'year_asc':
...
$fuelOptions = $db->fetchAll("SELECT DISTINCT fuel FROM vehicles WHERE status='active' ORDER BY fuel");
$yearMin = $db->fetchOne("SELECT MIN(CAST(year AS UNSIGNED)) as min FROM vehicles WHERE status='active'")['min'] ?? (date('Y') - 20);
$yearMax = $db->fetchOne("SELECT MAX(CAST(year AS UNSIGNED)) as max FROM vehicles WHERE status='active'")['max'] ?? date('Y');
...
<head>
    ...
    <meta name="description" content="İkinci el araç arama ve filtreleme. Fiyat, yıl, kilometre, yakıt tipi filtreleriyle aradığınız aracı bulun.">
    ...
    <style>
        ...
        .filter-group input, .filter-group select { width: 100%; padding: 0.8rem; border: 2px solid #e0e0e0; border-radius: 8px; font-size: 1rem; }
        .filter-group input:focus, .filter-group select:focus { outline: none; border-color: #ffd700; }
        .filter-actions { display: flex; gap: 1rem; margin-top: 1rem; flex-wrap: wrap; }
...
<body>
    ...
    <div class="container">
        <div class="search-container">
            <h1 style="color: #16213e; margin-bottom: 2rem;">🔍 Araç Arama ve Filtreleme</h1>
            
            <form method="GET" action="search.php">
                <div class="search-filters">
                    <div class="filter-group">
                        <label>🔎 Arama</label>
                        <input type="text" name="search" placeholder="Araç adı, marka, model..." value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>">
                    </div>
                    
                    <div class="filter-group">
                        <label>💰 Min Fiyat</label>
                        <input type="number" name="min_price" placeholder="0" min="0" step="10000" value="<?php echo htmlspecialchars($_GET['min_price'] ?? ''); ?>">
                    </div>
                    
                    <div class="filter-group">
                        <label>💰 Max Fiyat</label>
                        <input type="number" name="max_price" placeholder="10000000" min="0" step="10000" value="<?php echo htmlspecialchars($_GET['max_price'] ?? ''); ?>">
                    </div>
                    
                    <div class="filter-group">
                        <label>📅 Min Yıl</label>
                        <input type="number" name="min_year" min="<?php echo $yearMin; ?>" max="<?php echo $yearMax; ?>" placeholder="<?php echo $yearMin; ?>" value="<?php echo htmlspecialchars($_GET['min_year'] ?? ''); ?>">
                    </div>
                    
                    <div class="filter-group">
                        <label>📅 Max Yıl</label>
                        <input type="number" name="max_year" min="<?php echo $yearMin; ?>" max="<?php echo $yearMax; ?>" placeholder="<?php echo $yearMax; ?>" value="<?php echo htmlspecialchars($_GET['max_year'] ?? ''); ?>">
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <select name="transmission">
                            ...
                            <option value="Yarı Otomatik" <?php echo (isset($_GET['transmission']) && $_GET['transmission'] == 'Yarı Otomatik') ? 'selected' : ''; ?>>Yarı Otomatik</option>
                        </select>
                    </div>
            ...
            <div class="results-header">
                ...
                <form method="GET" style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <select name="sort" class="sort-select" onchange="this.form.submit()">
                        ...
                        <option value="km_asc" <?php echo (isset($_GET['sort']) && $_GET['sort'] == 'km_asc') ? 'selected' : ''; ?>>Kilometre (Az → Çok)</option>
            ...
            <?php if (empty($vehicles)): ?>
                <div style="text-align: center; padding: 4rem; background: white; border-radius: 10px;">
                    ...
                    <div style="margin-top: 2rem;">
                        <a href="search.php" class="btn">🔄 Tüm Araçları Göster</a>
                    </div>
                </div>
            <?php else: ?>
                        ...
                        <div class="vehicle-card" data-vehicle-id="<?php echo $vehicle['id']; ?>">
                            ...
                            <div class="vehicle-info">
                                ...
                                <?php if (!empty($vehicle['description'])): ?>
                                    <p style="color: #666; font-size: 0.9rem; margin-top: 0.5rem; line-height: 1.4;">
                                        <?php echo htmlspecialchars(mb_substr(strip_tags($vehicle['description']), 0, 100)); ?>
                                        <?php if (mb_strlen($vehicle['description']) > 100): ?>...<?php endif; ?>
                                    </p>
                                <?php endif; ?>
                                <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
                                    ...
                                    <?php endif; ?>
                                    <a href="https://wa.me/<?php echo htmlspecialchars($settings['whatsapp_number'] ?? '905321234567'); ?>?text=Merhaba,%20<?php echo urlencode($vehicle['title']); ?>%20hakkında%20bilgi%20almak%20istiyorum." target="_blank" class="btn" style="flex: 1; text-align: center; background: #25D366; font-size: 0.9rem; color: white;">💬 Teklif</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                ...
                <div style="text-align: center; margin-top: 3rem;">
                    <p style="color: #666; margin-bottom: 1rem;">
                        Toplam <?php echo count($vehicles); ?> araç gösteriliyor
                    </p>
                    <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                        <a href="index.php" class="btn" style="background: #6c757d; color: white;">← Ana Sayfaya Dön</a>
                        <a href="customer_add_vehicle.php" class="btn" style="background: #28a745; color: white;">+ Aracınızı Ekleyin</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
